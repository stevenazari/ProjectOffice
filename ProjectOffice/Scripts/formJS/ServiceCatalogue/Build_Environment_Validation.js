$(document).ready(function () {
    $('#Build_Environment_Form')
        .on('submit', function (e) {
            e.preventDefault();

            var Build_Environment_Submit = document.getElementById("Build_Environment_Submit");

            if (Build_Environment_Submit.classList.contains("disabled") == false) {
                submitForm(e, "buildEnvironmentResponse");
            }
        });
});

function buildEnvironmentResponse(message, results) {
    alert("here");
...
//        $("#environmentsTable tbody:first").after(rows);
    ...
    if (message == "Success") {
        $('#Build_Environment_Success_Message').slideDown({ opacity: "show" }, "slow"); // Do something ...
        $("#Build_Environment_Success_Message").fadeTo(5000, 500).slideUp(500, function () {
            $("#Build_Environment_Success_Message").slideUp(500);
        });
    } else {
        $('#Build_Environment_Error_Message').slideDown({ opacity: "show" }, "slow"); // Do something ...
        $("#Build_Environment_Error_Message").fadeTo(5000, 500).slideUp(500, function () {
            $("#Build_Environment_Error_Message").slideUp(500);
        });
    }
}